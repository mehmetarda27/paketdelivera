(() => {
  ...
  async function restaurantAccountingManagement() {
    const today = new Date().toISOString().slice(0, 10);
    const open = async (startDate = today, endDate = today) => {
      const result = await api(`/api/admin/accounting/restaurants?startDate=${encodeURIComponent(startDate)}&endDate=${encodeURIComponent(endDate)}`);
      const accounting = result.restaurantAccounting || [];
      const settlements = result.restaurantSettlements || [];
      modal("İşletme Tahsilat ve Hesap Yönetimi", `<form data-accounting-filter class="da-grid"><label class="da-field"><span>Başlangıç</span><input name="startDate" type="date" value="${esc(startDate)}"></label><label class="da-field"><span>Bitiş</span><input name="endDate" type="date" value="${esc(endDate)}"></label><div class="da-actions"><button class="da-secondary">Hesapları Getir</button></div></form><div class="da-list mt-5">${accounting.map((item) => { const paid = settlements.find((settlement) => settlement.restaurantId === item.restaurantId && settlement.startDate === startDate && settlement.endDate === endDate && settlement.status === "paid"); return `<div class="da-list-row" data-accounting-id="${esc(item.restaurantId)}"><div><b>${esc(item.restaurantName)}</b><small>${item.totalPackages} teslimat · Kurye tahsilatı ${money(item.totalCourierCollected)} · Hizmet bedeli ${money(item.serviceFee)}</small></div><div class="da-list-actions"><b>Net ${money(item.netPayable)}</b>${paid ? '<span class="da-badge">Ödendi</span>' : '<button data-accounting-paid>Ödendi İşaretle</button>'}</div></div>`; }).join("") || '<div class="da-empty">İşletme hesabı bulunamadı.</div>'}</div>`, (root) => {
        root.querySelector("[data-accounting-filter]").addEventListener("submit", (event) => { event.preventDefault(); const values = Object.fromEntries(new FormData(event.currentTarget)); open(values.startDate, values.endDate).catch((error) => toast(error.message, "error")); });
        root.querySelectorAll("[data-accounting-paid]").forEach((button) => button.addEventListener("click", async () => { const restaurantId = button.closest("[data-accounting-id]").dataset.accountingId; try { absorb(await api(`/api/admin/accounting/restaurants/${encodeURIComponent(restaurantId)}/mark-paid`, { method: "POST", body: JSON.stringify({ startDate, endDate, note: "Admin panelinden ödendi" }) })); toast("İşletme tahsilatı ödendi olarak kaydedildi.", "success"); await open(startDate, endDate); } catch (error) { toast(error.message, "error"); } }));
      });
    };
    await open();
  }
