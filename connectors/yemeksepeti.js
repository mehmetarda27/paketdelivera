...
const PLATFORM = "Yemeksepeti";
const ENV_PREFIX = "YEMEKSEPETI";
...
function endpoint(account, kind) {
  return trimmed(account?.settings?.[`${kind}Url`]) ||
    ...
    (kind === "orders" ? trimmed(process.env.YEMEKSEPETI_POLLING_URL) : "");
}
...
function authHeaders(account) {
  ...
  if (account?.externalStoreId) headers["x-vendor-id"] = account.externalStoreId;
  return headers;
}
