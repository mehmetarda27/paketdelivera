...
const PLATFORM = "Migros Yemek";
const ENV_PREFIX = "MIGROS";
...
function authHeaders(account) {
  ...
  if (account?.externalStoreId) headers["x-merchant-id"] = account.externalStoreId;
  return headers;
}

async function testConnection(account) {
  const url = endpoint(account, "verify") || endpoint(account, "orders");
  if (!url) return { ok: false, status: 404, message: "Migros verify/polling endpoint tanimli degil." };
  try {
...
async function fetchOrders(account) {
  ...
  if (!url) return [];
  const response = await fetch(url, { method: "GET", headers: authHeaders(account) });
  if (!response.ok) throw new Error(`Migros polling HTTP ${response.status}`);
